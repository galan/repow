...
func processDir(dirReposRoot string, hoster h.Hoster, counter *int32, total int, counterOk *int32, counterSkipped *int32, counterArchived *int32, counterRemoved *int32, tasks chan model.RepoDir, wg *sync.WaitGroup) {
	...
	for dirRepository := range tasks {
		...
		displayName := dirRepository.RepoMeta.RemotePath

		remotePath := model.DetermineRemotePath(dirRepository.Path, hoster.Host())

		if remotePath == "" {
			say.ProgressWarn(counter, total, nil, displayName, "- Unable to determine git remote name (skipping)")
			atomic.AddInt32(counterSkipped, 1)
			continue
		}

		say.Verbose("RemotePath: %s: %s", displayName, remotePath)
		state, err := hoster.ProjectState(remotePath)
		if err != nil {
			say.ProgressWarn(counter, total, err, displayName, "- Unable to determine git remote state (skipping)")
			atomic.AddInt32(counterSkipped, 1)
			continue
		}
		say.Verbose("State for %s: %v", displayName, state)

		var errorMove error = nil
		var code string = ""
		switch state {
		case h.Ok:
			say.Verbose("Repository %s ok", displayName)
		...
		default:
			say.ProgressError(counter, total, err, displayName, "- State for repository is unknown (skipping)")
		...
		if errorMove != nil {
			say.ProgressError(counter, total, errorMove, displayName, "- Unable to move")
		} else {
			if !cleanupQuiet || state != h.Ok {
				say.ProgressGeneric(counter, total, code, displayName, "")
			}
		}
	}
}
...
func move(dirReposRoot string, dirRepository string, dirTarget string) error {
	dirRepoRelative := strings.TrimPrefix(dirRepository, dirReposRoot) // leading "/" doesn't matter
